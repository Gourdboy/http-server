      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          const writeRow = (file) => {
            ...
            if (!hidePermissions) {
              html += `<td class="perms"><code>(${permsToString(
                file[1]
              )})</code></td>`;
            }
            html +=
              `<td class="last-modified">${lastModifiedToString(
                file[1]
              )}</td>` +
              `<td class="file-size"><code>${sizeToString(
                file[1],
                humanReadable,
                si
              )}</code></td>` +
              `<td class="display-name"><a href="${href}">${displayName}</a>${
                !isDir ? ` <a href="${href}" download>Download</a>` : ''
              }</td>` +
              '</tr>\n';
          };
          ...
          html += '</table>\n';
          html +=
            `<br><address>Node.js ${process.version}/ <a href="https://github.com/http-party/http-server">http-server</a> ` +
